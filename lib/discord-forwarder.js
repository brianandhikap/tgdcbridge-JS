  ...
  async sendMessage(webhookUrl, messageData) {
    try {
      ...
      if (messageData.avatarUrl) {
        if (messageData.avatarUrl.startsWith("http://") || messageData.avatarUrl.startsWith("https://")) {
          // Use HTTP URL directly
          avatarUrl = messageData.avatarUrl
          console.log(`🖼️ Using hosted avatar: ${avatarUrl}`)
        } else if (await this.fileExists(messageData.avatarUrl)) {
          // Use local file and convert to base64
          avatarUrl = await this.uploadAvatarToDiscord(messageData.avatarUrl, webhookUrl)
        }
      }

      // If no avatar found, use default profile picture
      if (!avatarUrl) {
