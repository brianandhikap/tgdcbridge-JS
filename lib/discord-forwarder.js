...
class DiscordForwarder {
  ...
  async testWebhookWithMessage(webhookUrl) {
    try {
      ...
      if (await this.fileExists(defaultPP)) {
        // For Discord webhooks, we need to use a hosted URL or upload the file
        // Since we can't use base64 avatars, we skip for test
        // but include it in the actual message sending
        console.log(`📸 Profile picture found: ${defaultPP}`)
      }

      const testPayload = {
        username: process.env.BDN || "Test Bot",
        content: process.env.SW || "BOT Started",
        // Note: Discord webhooks have issues with base64 avatars, so we skip for test
      }

      await this.sendTextMessage(webhookUrl, testPayload)
      console.log(`✅ Webhook test successful (profile picture will work in actual forwarding)`)
      return true
    } catch (error) {
      console.error(`❌ Webhook test failed for ${webhookUrl}:`, error.message)
      return false
