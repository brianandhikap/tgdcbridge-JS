...
class TelegramBot {
  ...
  async handleNewMessage(update) {
    ...
    console.log(`[v0] Database query: groupId=${groupId}, topicId=${topicId}`)
    console.log(`[v0] Found routing:`, routing ? `Yes - ${routing.DC_Webhook.substring(0, 50)}...` : "No")

    if (!routing) {
      console.log(`📝 No routing found for group ${groupId}, topic ${topicId || "N/A"}`)
      return
    }
