...
class TelegramBot {
  ...
  async createFallbackSenderInfo(userId) {
    ...
    const defaultAvatarUrl = `http://${host}:${port}/ava/pp.jpg`

    // Copy default profile picture to ava folder if it doesn't exist
    try {
      const avaDir = path.join(__dirname, "../ava")
      await fs.mkdir(avaDir, { recursive: true })

      const defaultPath = path.join(avaDir, "pp.jpg")
      const ppPath = path.join(__dirname, "../img/PP.png")

      try {
        await fs.access(defaultPath)
      } catch {
        // Copy PP.png to ava/default.jpg if it doesn't exist
        try {
          const ppBuffer = await fs.readFile(ppPath)
          await fs.writeFile(defaultPath, ppBuffer)
          console.log(`   📋 Copied default profile picture to ava folder`)
        } catch (error) {
      ...
      try {
        if (sender.photo) {
          ...
          if (buffer) {
            ...
            const filename = `${senderInfo.username}.jpg`
